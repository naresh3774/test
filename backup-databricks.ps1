...
function Export-WorkspaceComplete {
    ...
    try { databricks workspace export-dir $SourcePath $LocalFolder --profile $DatabricksProfile } catch { Write-Warning "Failed export folder $SourcePath. $_" }

    try {
        $items = databricks workspace list $SourcePath --profile $DatabricksProfile | ConvertFrom-Json
...
Export-Json "databricks jobs list --profile $DatabricksProfile" (Join-Path $BackupRoot "jobs.json")
$ClustersJson = Join-Path $BackupRoot "clusters.json"
Export-Json "databricks clusters list --profile $DatabricksProfile" $ClustersJson
Export-Json "databricks cluster-policies list --profile $DatabricksProfile" (Join-Path $BackupRoot "cluster-policies.json")
Export-Json "databricks instance-pools list --profile $DatabricksProfile" (Join-Path $BackupRoot "instance-pools.json")
Export-Json "databricks global-init-scripts list --profile $DatabricksProfile" (Join-Path $BackupRoot "global-init-scripts.json")
...
foreach ($cluster in $clusters) {
    try {
        ...
        Export-Json "databricks libraries list --cluster-id $($cluster.cluster_id) --profile $DatabricksProfile" $libFile
    } catch { Write-Warning "Failed libraries for cluster $($cluster.cluster_id)" }
}

# --- Secret Scopes ---
Export-Json "databricks secrets list-scopes --profile $DatabricksProfile" (Join-Path $BackupRoot "secret-scopes.json")

# --- Users and Groups (admin token required) ---
Export-Json "databricks scim list-users --profile $DatabricksProfile" (Join-Path $BackupRoot "users.json")
Export-Json "databricks scim list-groups --profile $DatabricksProfile" (Join-Path $BackupRoot "groups.json")

# --- Repos ---
Export-Json "databricks repos list --profile $DatabricksProfile" (Join-Path $BackupRoot "repos.json")
